...
[^1]: These Tcl programs were interpreted in a restricted interpreter that had just the equivalent of `BEGIN`, `INSERT`, `UPDATE`, `DELETE`, `COMMIT`, and DDL statements for making schema changes.  Thus UName\*It had something like JSON, but for Tcl, well before JSON.
